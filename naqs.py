...
class WaveFunction(torch.nn.Module):
    ...
    def generate(self, batch_size):
        ...
        return x.reshape([x.size(0), self.double_sites]), real_amplitude, real_probability, multiplicity
    ...
    def generate_unique(self, batch_size):
        ...
        for i in range(self.sites):
            ...
            xf = x.to(dtype=dtype)
            delta_log_amplitude = self.amplitude[i](xf.reshape([local_batch_size, 2 * i])).reshape([local_batch_size, 2, 2])
            delta_log_amplitude = delta_log_amplitude + torch.where(self.mask(x), 0, -torch.inf)
