...
template<int n_qubits, int n_values>
std::int64_t ensure_impl(const torch::Tensor& key, torch::Tensor& value, std::int64_t length_config) {
...
template<typename NQubits, typename NValues>
auto merge_interface(const torch::Tensor& key_1, const torch::Tensor& value_1, torch::Tensor& key_2, torch::Tensor& value_2)
